...
async function use({ workflow }) {
    ...
    let sceneEchos = [];
    if (effect)
        sceneEchos = effect.flags["chris-premades"].summons.ids[effect.name].map((i) =>
            workflow.token.scene.tokens.get(i),
        );
    let teleportFeature = activityUtils.getActivityByIdentifier(workflow.item, "manifestEchoTeleport", {
        strict: true,
    });
    ...
    if (!name?.length)
        name = genericUtils.format("CHRISPREMADES.Macros.ManifestEcho.Echo", { name: workflow.token.name });
    let updates = {
        actor: {
            name,
            system: {
    ...
    let vaeButtons = [
        {
            ...
            activityIdentifier: "manifestEchoTeleport",
        },
    ...
    if (unleashIncarnation)
        vaeButtons.push({ type: "use", name: unleashIncarnation.name, identifier: "unleashIncarnation" });
    let spawnedTokens = await Summons.spawn(
...
async function teleport({ workflow }) {
    let effect = effectUtils.getEffectByIdentifier(workflow.actor, "manifestEcho");
    if (!effect) return;
    let sceneEchos = effect.flags["chris-premades"].summons.ids[effect.name]
    ...
    if (sceneEchos.length > 1) {
        let selection = await dialogUtils.selectTargetDialog(
            workflow.item.name,
            "CHRISPREMADES.Macros.ManifestEcho.Which",
            sceneEchos,
        );
    ...
    let origTeleport = new Teleport([workflow.token], workflow.token, {
...
export async function attack({ workflow }) {
    ...
    if (features.length > 1) {
        feature = await dialogUtils.selectDocumentDialog(
            workflow.item.name,
            "CHRISPREMADES.Macros.ManifestEcho.Weapon",
            features,
        );
        if (!feature) return;
    } else {
    ...
    for (let i of sceneEchos) {
        ...
        if (distance > maxRange) {
            let selection = await dialogUtils.confirm(
                effect.name,
                genericUtils.format("CHRISPREMADES.Macros.ManifestEcho.Far", { actorName: token.actor.name }),
...
async function targetApplyDamage({ trigger, ditem }) {
    ...
    let selection = await dialogUtils.confirm(
        originItem.name,
        genericUtils.format("CHRISPREMADES.Dialog.Use", { itemName: originItem.name }),
    );
    if (!selection) return;
